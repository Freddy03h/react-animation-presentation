...
const FlipMove = propConverter(class FlipMove extends Component {
  constructor(props) {
    super(props);
    this.boundingBoxes = {};
  ...
  calculateAndAnimateChildren() {
    ...
    if ( this.props.leaveAnimation ) {
      ...
      leavingChildren.forEach( leavingChild => {
        ...
        domNode.style.width  = leavingBoundingBox.width + 'px';
        domNode.style.height  = leavingBoundingBox.height + 'px';
      });
    }
  ...
  computeInitialStyles(child) {
    ...
    if ( child.entering ) {
      ...
      const [ dX, dY, sX, sY ]  = this.getPositionDelta(domNode, child.key);
